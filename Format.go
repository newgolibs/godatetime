...
func (this Format) IsLeapYear() bool {
	year := this.Time.Year()
...
func (this Format) DayOfYear() int {
	day := this.Time.Day()
	month := int(this.Time.Month())
	if this.IsLeapYear() {
		if month > 2 {
...
func (this Format) DaysInMonth() int {
...
func (this Format) Invoke() string {
	...
	for i := 0; i < len(runes); {
		...
		case 'W':
			_, week := this.Time.ISOWeek()
			buffer.WriteString(strconv.Itoa(week))
		case 'z':
			buffer.WriteString(strconv.Itoa(this.DayOfYear()))
		case 't':
			buffer.WriteString(strconv.Itoa(this.DaysInMonth()))
		case 'U':
			buffer.WriteString(strconv.FormatInt(this.Time.Unix(), 10))
		default:
			if runes[i] > 255 {
